...
COMMIT_SUBJECT="$(docker compose exec --no-TTY server bash -ci "python -m src.commit_message --filename ${ATOM_PATH}")"
...
git init
git config user.email "marcindulak@users.noreply.github.com"
git config user.name "Marcin Dulak"
