...
Feature: FR-005 Weekly storage path and week-rollover archive
  ...
  Scenario Outline: The output file path uses a lowercased category
    ...
    When the archive-and-write step runs in a fresh temporary directory
  ...
  Scenario: First run does not create an archive file
    ...
    When the archive-and-write step runs
    Then no file exists under "docs/arxiv/cs.ai/archive/"

  Scenario: Existing file from a prior ISO week is archived under that prior week's directory
    Given an existing "docs/arxiv/cs.ai/atom.xml" whose newest entry published date is "2026-05-08T12:00:00Z"
    And the environment variable PIPELINE_TODAY is "2026-05-14"
    When the archive-and-write step runs
    Then the file "docs/arxiv/cs.ai/archive/2026-W19/atom.xml" exists
    And the contents of "docs/arxiv/cs.ai/archive/2026-W19/atom.xml" match the prior contents of "docs/arxiv/cs.ai/atom.xml"
